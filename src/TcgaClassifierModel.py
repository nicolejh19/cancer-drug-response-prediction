...
class ModelTCGAClassifier():
    def __init__(self, drug_name, input_name):
        self.drug_name = drug_name
        self.input_name = input_name
        self.model_path = f'./benchmark_models/{input_name}/{drug_name}/tuned/'
        if not os.path.isdir(self.model_path):
            os.makedirs(self.model_path)
        self.logs = f'./benchmark_logs/{input_name}/{drug_name}/tuned/'
        if not os.path.isdir(self.logs):
            os.makedirs(self.logs)
        self.prediction_path = f'./benchmark_predictions/{input_name}/{drug_name}/tuned/'
    ...
    def setup_path(self, flags):        
        ...
        print(f'-------{drug_name}-------')
        for t in temp_paths:
            train_df = get_dataset_benchmark(t, drug_name, self.input_name, 'train')
        ...
        self.val_paths = []
        for t in temp_paths:
            val_df = get_dataset_benchmark(t, drug_name, self.input_name, 'val')
        ...
        self.test_paths = []
        for t in temp_paths:
            test_df = get_dataset_benchmark(t, drug_name, self.input_name, 'test')
...
#         self.loss_fn = crossentropyloss().cuda()
    ...
    def train_target_nn(self, flags):
...
#         self.bn_process(flags) # set to eval all batch norms
    ...
    def tune_model(self, parameters, total_trials):
        best_parameters, values, experiment, model = optimize(
            experiment_name="tcga_classifier_benchmark",
    ...
    def get_results(self, best_parameters):
        ...
        self.target_nn.load_state_dict(torch.load(os.path.join(self.model_path, f'Best_NN.pt')))
        self.target_nn.eval()
...
#         self.bn_process(best_parameters)
    ...
    def get_train_set_results(self, best_parameters, train_model):
        ...
        for t in train_all_paths:
            train_all_datasets.append(get_dataset_benchmark(t, self.drug_name, self.input_name, 'train_all'))
...
#             print('After Sigmoid', predictions)
    ...
    def get_roc_pr_curves(self, best_parameters, train_model, dataset):
        ...
        if train_model:
            self.train_evaluate(best_parameters, save_model=True)
        self.target_nn.load_state_dict(torch.load(os.path.join(self.model_path, 'Best_NN.pt')))
        self.target_nn.eval()
