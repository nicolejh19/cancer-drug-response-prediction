...
def get_cl_aadrc(drug_name, type_set):
    random.seed(42)
    np.random.seed(42)
...
#     cl_train_merged_df['response'] = cl_train_merged_df['response'].astype(int)
    cl_train_merged_df['response'] = 1 - cl_train_merged_df['auc']
...
def get_unlabelled_tcga(drug_name):
    tcga_train = AggCategoricalAnnotatedTcgaDataset(
        is_train=True,
        only_cat_one_drugs=True,
    )
    all_tcga_train_response_drug_df = tcga_train.tcga_response 
    drug_tcga_train_response_df = tcga_train.tcga_response[tcga_train.tcga_response["drug_name"] == drug_name] # has depmap_id, drug_name, auc 
    subset_all_tcga_train_mut_df = tcga_train.raw_mutations[(tcga_train.raw_mutations.index.isin(all_tcga_train_response_drug_df.submitter_id)) & ~(tcga_train.raw_mutations.index.isin(drug_tcga_train_response_df.submitter_id))].copy()
    labelled_tcga = pd.merge(subset_all_tcga_train_mut_df.reset_index(), all_tcga_train_response_drug_df.drop("drug_name", axis = 1), on = "submitter_id")
    labelled_tcga = labelled_tcga.drop_duplicates(subset='submitter_id').set_index("submitter_id")
    return labelled_tcga

def get_cl_cnv_aadrc(drug_name, type_set):
    random.seed(42)
    np.random.seed(42)
...
def get_unlabelled_tcga_cnv(drug_name):
    tcga_train = AggCategoricalAnnotatedTcgaDataset(
        is_train=True,
        only_cat_one_drugs=True,
    )
    all_tcga_train_response_drug_df = tcga_train.tcga_response 
    drug_tcga_train_response_df = tcga_train.tcga_response[tcga_train.tcga_response["drug_name"] == drug_name] # has depmap_id, drug_name, auc 
    subset_all_tcga_train_cnv_df = tcga_train.cnv[(tcga_train.cnv.index.isin(all_tcga_train_response_drug_df.submitter_id)) & ~(tcga_train.cnv.index.isin(drug_tcga_train_response_df.submitter_id))].copy()
    labelled_tcga = pd.merge(subset_all_tcga_train_cnv_df.reset_index(), all_tcga_train_response_drug_df.drop("drug_name", axis = 1), on = "submitter_id")
    labelled_tcga = labelled_tcga.drop_duplicates(subset='submitter_id').set_index("submitter_id")
    return labelled_tcga

def get_cl_mut_cnv_aadrc(drug_name, type_set):
    random.seed(42)
    np.random.seed(42)
...
def get_unlabelled_tcga_mut_cnv(drug_name):
    tcga_train = AggCategoricalAnnotatedTcgaDataset(
        is_train=True,
        only_cat_one_drugs=True,
    )
    ...
    subset_all_tcga_train_mut_df = tcga_train.raw_mutations[(tcga_train.raw_mutations.index.isin(all_tcga_train_response_drug_df.submitter_id)) & ~(tcga_train.raw_mutations.index.isin(drug_tcga_train_response_df.submitter_id))].copy()
    subset_all_tcga_train_cnv_df = tcga_train.cnv[(tcga_train.cnv.index.isin(all_tcga_train_response_drug_df.submitter_id)) & ~(tcga_train.cnv.index.isin(drug_tcga_train_response_df.submitter_id))].copy()
    subset_all_tcga_train_mut_cnv_df = pd.merge(subset_all_tcga_train_mut_df.reset_index(), subset_all_tcga_train_cnv_df.reset_index(), on = "submitter_id").set_index("submitter_id")
    labelled_tcga = pd.merge(subset_all_tcga_train_mut_cnv_df.reset_index(), all_tcga_train_response_drug_df.drop("drug_name", axis = 1), on = "submitter_id")
    labelled_tcga = labelled_tcga.drop_duplicates(subset='submitter_id').set_index("submitter_id")
    return labelled_tcga

def get_dataset_decota(domain_name, drug_name, input_name, type_set=''):
    if input_name == 'mutation':
        if domain_name == 'cl':
            return get_cl_aadrc(drug_name, type_set)
        elif domain_name == 'pdx':
            return get_pdx(drug_name, type_set)
        elif domain_name == 'tcga':
            return get_tcga(drug_name, type_set)
        elif domain_name == 'unlabelled_tcga':
            return get_unlabelled_tcga(drug_name)
        else:
            raise ValueError(
        ...
        if domain_name == 'cl':
            return get_cl_cnv_aadrc(drug_name, type_set)
        elif domain_name == 'pdx':
            return get_pdx_cnv(drug_name, type_set)
        elif domain_name == 'tcga':
            return get_tcga_cnv(drug_name, type_set)
        elif domain_name == 'unlabelled_tcga':
            return get_unlabelled_tcga_cnv(drug_name)
        else:
            raise ValueError(
                        f"Unsupported domain - {domain_name} - accepted types are [cl, pdx, tcga] "
                    ) 
        ...
        if domain_name == 'cl':
            return get_cl_mut_cnv_aadrc(drug_name, type_set)
        elif domain_name == 'pdx':
            return get_pdx_mut_cnv(drug_name, type_set)
        elif domain_name == 'tcga':
            return get_tcga_mut_cnv(drug_name, type_set)
        elif domain_name == 'unlabelled_tcga':
            return get_unlabelled_tcga_mut_cnv(drug_name)
        else:
            raise ValueError(
                        f"Unsupported domain - {domain_name} - accepted types are [cl, pdx, tcga] "
                    ) 
