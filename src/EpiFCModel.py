...
# changes made for ablation study
...
class ModelEpiFC():
    def __init__(self, drug_name, input_name):
        ...
        self.model_path = f'./epi_ablation_fc_models/{input_name}/{drug_name}/tuned/'
        if not os.path.isdir(self.model_path):
            os.makedirs(self.model_path)
        self.logs = f'./epi_ablation_fc_models_logs/{input_name}/{drug_name}/tuned/'
        if not os.path.isdir(self.logs):
            os.makedirs(self.logs)
        self.prediction_path = f'./epi_ablation_fc_models_predictions/{input_name}/{drug_name}/tuned/'
...
#         self.batGenTest = BatchGenerator(flags=flags, file_path=self.unseen_data_path, stage='test')
    ...
    def train_agg_nn(self, ite, flags): # ite = num_iter in training loop
        ...
        for index in range(len(self.batGenTrainsDg)):
            ...
            # forward
            outputs_agg, _ = self.agg_nn(x=inputs)
        ...
        (agg_ent_loss + epic_loss + epif_loss).backward()
    ...
    def train_agg_nn_warm(self, ite, flags):
        ...
        for index in range(len(self.batGenTrainsDg)):
            ...
            # forward
            outputs, _ = self.agg_nn(x=inputs)
